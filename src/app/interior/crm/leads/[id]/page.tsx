...
export default function Lead360View() {
  ...
  const fetchData = async () => {
    try {
      const leadRes = await api.get('/crm/customers');
      const found = leadRes.data.find((d: any) => d._id === params.id);
      if (found) setLead(found);

      const actRes = await api.get(`/crm/activities?customerId=${params.id}`);
  ...
  const handleStatusChange = async (newStatus: string) => {
    try {
      await api.patch(`/crm/customers/${params.id}`, { status: newStatus });
  ...
  if (isLoading) return <Shell><div className="p-8 flex items-center justify-center text-slate-500 min-h-[60vh] font-medium animate-pulse">Loading Lead Profile...</div></Shell>;
  if (!lead) return <Shell><div className="p-8 text-rose-500 font-bold text-center">Lead not found.</div></Shell>;
      ...
      <div className="max-w-7xl mx-auto space-y-6 pb-24 p-4 md:p-8 animate-in fade-in duration-500">
        ...
        <div className="bg-white rounded-[2rem] p-6 md:p-8 shadow-sm border border-slate-200 flex flex-col md:flex-row md:items-center justify-between gap-6 relative overflow-hidden group">
          ...
          <div className="flex items-start md:items-center gap-5 z-10">
            <button 
              onClick={() => router.push('/interior/crm')}
