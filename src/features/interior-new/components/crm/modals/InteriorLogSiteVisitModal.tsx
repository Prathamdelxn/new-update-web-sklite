...
export const InteriorLogSiteVisitModal = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(error?.response?.data?.message || error?.message || 'Failed to save site visit');
    } finally {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="md:col-span-2">
                ...
                <textarea
                  ...
                  className={`w-full bg-[hsl(var(--muted))] border rounded-xl px-3.5 py-2.5 text-xs text-[hsl(var(--foreground))] outline-none transition-all h-20 resize-none placeholder:text-[hsl(var(--muted-foreground)/0.6)] ${
                    errors.notes
                      ? 'border-red-500 focus:ring-2 focus:ring-red-500/20'
                      : 'border-[hsl(var(--border))] focus:ring-2 focus:ring-[hsl(var(--ring))]'
                  }`}
