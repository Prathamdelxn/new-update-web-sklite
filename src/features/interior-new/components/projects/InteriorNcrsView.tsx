...
import { AlertTriangle, Plus, X, Loader2, FileText } from 'lucide-react';
...
export default function InteriorNcrsView({ projectId }: InteriorNcrsViewProps) {
  ...
  const handleCreateNCR = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await interiorProjectService.createNcr(projectId, payload);
      if (res?.success) {
        toast.success('Non-Conformance Report logged successfully!');
